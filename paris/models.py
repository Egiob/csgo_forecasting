...
class Prediction(models.Model):
	...
	delta_ev_1 = models.DecimalField(max_digits= 100, decimal_places = 2,default=0)
	delta_ev_2 = models.DecimalField(max_digits= 100, decimal_places = 2,default=0)
	model =  models.CharField(max_length=200, choices = MODELS)
	def __str__(self):
		return "Prediction of %s" %(self.match)


#TODO : changer le one to one en many to one
class Bet(models.Model):
	class Meta:
		constraints = [
        models.UniqueConstraint(fields=['match', 'strategy'], name='unique strat')]
	match = models.ForeignKey(Match, on_delete=models.CASCADE)
	winner = models.CharField(max_length=200,choices=WINNER,default='None')
	amount = models.DecimalField(max_digits= 10, decimal_places = 2,default=0)
	status = models.CharField(max_length=200,choices=BET_STATUS,default='Pending')
	gain = models.DecimalField(max_digits=10,decimal_places = 5,default = 0)
	real = models.BooleanField(default=False)
	strategy = models.CharField(max_length=200,choices=BET_STRATEGY,default='EV')
	bet_id = models.AutoField(primary_key=True)

	def __str__(self):
		return "%d €  on %s" %(self.amount,self.match.team1 * (self.winner == 'Team 1') +self.match.team2 * (self.winner=='Team 2'))
